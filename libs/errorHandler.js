...
const errorHandler = async (error, res, req) => {
  const statusCode = error?.response?.statusCode ? error.response.statusCode : DEFAULT_ERROR.STATUS_CODE;
  if (error instanceof CustomError) {
    return res.status(statusCode).json({
      message: error?.response?.message ? error.response.message : DEFAULT_ERROR.MESSAGE
    });
  } else {
